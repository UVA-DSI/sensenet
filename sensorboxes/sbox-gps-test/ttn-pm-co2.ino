...
void gps_get_fix(Adafruit_GPS &gps) {
  // keep doing this until you get a fix
  // capped at a maximum of 3 attempts
  int tries = 3;
  while (!gps.fix) {
    char c = gps.read();
    ...
    if (GPS.newNMEAreceived()) {
          ...
          #ifdef DEBUG
          Serial.println("received sentence");
          #endif
...
#ifdef DEBUG
  Serial.println("Starting up GPS");
#endif

  // gps enable pin
  pinMode(gpsENpin, OUTPUT);
  digitalWrite(gpsENpin, HIGH); // enable the gps
...
#ifdef DEBUG
  Serial.println("Fix acquisition process done");
#endif
  
  #ifdef DEBUG
    if (GPS.fix) Serial.println("GPS fix successful");
    else Serial.println("GPS fix failed, fix invalid");
  #endif

  delay(1000);
  
  GPS.standby();
  digitalWrite(gpsENpin, LOW);  // disable it
